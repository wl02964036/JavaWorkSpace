...
import ch05.one2many._04.anno.model.Employee;
import ch05.one2many._04.anno.util.HibernateUtils;
//雙向一對多: 由部門(Department)來找出其內所有員工(Employee)，也可以由員工(Employee)
//           找到其所屬部門(Department)。
//口訣: 一對多，一方有個儲存多方物件參考的Set/List物件，簡稱『ㄧ方有個多』
//口訣: 多對一，多方有個儲存一方物件參考的實例變數，簡稱『多方有個ㄧ』
// 
//只要部門類別內含有一個能儲存多方物件(員工類別)的Set/List物件，程式就可以由部門找到其內
//所有員工。
//Employee類別內有一個欄位儲存該Employee物件所屬之部門的物件參考，
// Hibernate就可以由員工(Employee)來找出其所屬的部門(Department)
...
public class O2MMain04_Bi_Insert {
	...
	public static void main(String[] args) {
		Employee emp1 = new Employee(null, "GAM001", "劉敏珍-BI");
		Employee emp2 = new Employee(null, "GAM002", "湯元泰-BI");
		Set<Employee> set1 = new HashSet<>(Arrays.asList(emp1, emp2));
		Department dept1 = new Department(null, "GAM_A", "遊戲部-BI", set1);
		emp1.setDept(dept1);
		emp2.setDept(dept1);
		// ---------------------------------------------------------------
		Employee emp3 = new Employee(null, "RES001", "林信民-BI");
		Employee emp4 = new Employee(null, "RES002", "吳雅芳-BI");
		Employee emp5 = new Employee(null, "RES003", "陳智勝-BI");
		Set<Employee> set2 = new HashSet<>(Arrays.asList(emp3, emp4, emp5));
		Department dept2 = new Department(null, "RES_A", "餐飲部-BI", set2);
		emp3.setDept(dept2);
		emp4.setDept(dept2);
		emp5.setDept(dept2);
		// ---------------------------------------------------------------
		Employee emp6 = new Employee(null, "TRA001", "莊淑芬-BI");
		Set<Employee> set3 = new HashSet<>(Arrays.asList(emp6));
		Department dept3 = new Department(null, "ENG_A", "旅遊部-BI", set3);
		emp6.setDept(dept3);
