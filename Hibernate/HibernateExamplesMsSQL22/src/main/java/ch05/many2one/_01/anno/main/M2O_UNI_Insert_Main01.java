...
//單向多對一: 由員工(Employee)來找出他的雇主(Employer)，但無法由雇主
//找到員工。
...
public class M2O_UNI_Insert_Main01 {
	public static void main(String[] args) {
		...
		Employer emperA = new Employer("劉瑪莉老闆");	// 雇主類別
		Employee empeeA1 = new Employee("劉小明"); 		// 員工類別
		empeeA1.setEmployer(emperA);
		dao.save(empeeA1);
		Employee empeeA2 = new Employee("劉美華");		// 員工類別
		empeeA2.setEmployer(emperA);
		dao.save(empeeA2);
		
		Employer emperB = new Employer("林芳華老闆");	// 雇主類別
		Employee empeeB1 = new Employee("林偉明"); 		// 員工類別
		empeeB1.setEmployer(emperB);
		dao.save(empeeB1);
		Employee empeeB2 = new Employee("林世光");		// 員工類別
		empeeB2.setEmployer(emperB);
		dao.save(empeeB2);
		
		Employer emperC = new Employer("黃河新老闆");	// 雇主類別
		Employee empeeC1 = new Employee("黃天南"); 		// 員工類別
