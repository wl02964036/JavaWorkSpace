...
@WebServlet("/ch04/ex01/register0401JDBC.do")
public class Register0401JDBC extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
		...
		if (expericnceStr == null || expericnceStr.trim().length() == 0) {
			errorMsg.put("experience", "使用Java經驗欄必須輸入");
		} else {
			try {
		...
		if (experience < 0) {
			errorMsg.put("userId", "使用Java經驗應該為正整數或 0 ");
		}
		if (!errorMsg.isEmpty()) {
			RequestDispatcher rd = request.getRequestDispatcher("ch0401_JDBC.jsp");
			rd.forward(request, response);
			return;
		}
		// 4. 進行 Business Logic 運算
		MemberService ms = new MemberServiceImpl_Jdbc();    // 使用JDBC技術
		...
		if (errorMsg.isEmpty()) {
			response.sendRedirect(request.getContextPath() + "/ch04/ex01/success.jsp");  
			return;
		} else {
			RequestDispatcher rd = request.getRequestDispatcher("ch0401_JDBC.jsp");
			rd.forward(request, response);
