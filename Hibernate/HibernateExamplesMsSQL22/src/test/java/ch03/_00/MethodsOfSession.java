...
public class MethodsOfSession {
	...
	// 利用persist()儲存的臨時物件絕對不能有OID(因為會當掉)，
...
//	--------------------------------------------------------
	...
	public void deleteDemo02() {
		...
		session.delete(bb); // 不可以用session.delete()方法來刪除臨時物件，會丟出例外。
	...
	public void deleteDemo04() {
		...
		session.delete(bb2);	//主要是看該primary key是否存在
	}

	// ====================================== ===================
	@Test
	// Session#update() 用來更新分離物件。	//只有分離物件可以寫session.update(); 
...
// 如果傳入的參數為 臨時(transient)物件，執行新增操作，執行INSERT敘述
	...
	public void saveOrUpdateDemo01() {
		...
		if (bb1 == null) {
			System.out.println("BookBean物件不存在，調整主鍵值");
			return;
		}
		tx.commit();
		session.close();
	...
	public void mergeDemo02() {
		...
		// OK, 會發出UPDATE的SQL敘述，更新表格中主鍵等於OID的紀錄
...
//		session.flush();
	...
	public void mergeDemo03() {
		...
		session.save(bb3);  // OK, 與session.merge()完全相同，即會發出INSERT INTO的SQL敘述
		                    // ，忽略原有的OID
