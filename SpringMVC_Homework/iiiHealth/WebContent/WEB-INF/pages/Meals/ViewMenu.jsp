<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8" import="java.util.*,tw.iiihealth.meals.*"%>
	...
	<div id="layoutSidenav">
			...
			<div class="sb-sidenav-menu">
				<div class="nav">
					...
					<div class="collapse" id="collapseLayouts3"
						...
						<a class="nav-link" href="../diet/dietView.controller">食品資料管理</a> </nav>
					</div>
					<a class="nav-link collapsed" href="#" data-bs-toggle="collapse"
		...
		<div id="layoutSidenav_content">
			...
			<div class="card-header">
				...
				<%
					ArrayList<MealBean> MB = (ArrayList<MealBean>) session.getAttribute("queryAll");
				%>
				<%
					for (MealBean item : MB) {
				%>
				<form>
					<fieldset>
						<label id="mb">餐點名稱：<%=item.getName()%></label> <br> <label
							id="mb">價格：<%=item.getPrice()%></label> <br> <label id="mb">主原料：<%=item.getMaterial()%></label>
						<br> <label id="mb">熱量(kcal)：<%=item.getCalories()%></label>
						<br> <label id="mb">蛋白質(g)：<%=item.getProtein()%></label> <br>
						<label id="mb">碳水化合物(g)：<%=item.getCarbohydrates()%></label> <br>
						<label id="mb">脂肪(g)：<%=item.getFat()%></label> <br> <label
							id="mb">商店編號：<%=item.getStore_ID()%></label> <br>
					</fieldset>
					<br>
				</form>
				<%
